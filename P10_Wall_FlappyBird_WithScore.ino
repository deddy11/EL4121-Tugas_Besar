...
void loop(void){
  ...
  for( ;; ){
    ...
    x3++;
    x4++;
    // atur kasus agar setiap kali posisi wall mencapai maksimal, maka ia kembali ke posisi awal x = 0
    // saat kembali ke posisi x = 0, tinggi dari setiap wall juga diubah dengan tinggi yang baru
    // setiap kali ada wall yang kembali ke posisi x = 0, skor bertambah 1
    if(x1 >= 32) {
      x1 = 0;
      tinggibwh1 = random(0, 11);
      skor++;
